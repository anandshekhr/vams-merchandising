...
def homePage(request):
    try:                                                      
        products_dealoftheday = Products.objects.filter(category__contains = ['Deal of the day'])
...
def productDetailsPageView(request,pk,category_name):
    ...
    rating = ProductReviewAndRatings.objects.filter(product=product).aggregate(Avg('ratings'))
    
    #for greyed stars
    nonrating = 5 - int(rating['ratings__avg'] if rating['ratings__avg'] is not None else 0)

    #filter related products
    category = Categories.objects.get(category_name=category_name)
    related_products = Products.objects.filter(category__contains=[category.category_name]).exclude(pk=pk)
    related_product_ratings = ProductReviewAndRatings.objects.filter(product__in=related_products).aggregate(Avg('ratings'))
    ...
    return render(request,"single-product-1.html",context)
...
class ProductDetailsAPI(APIView):
    ...
    def get(self, request, product_id,format=None):
        ...
        review_serializer = ProductReviewAndRatingsSerializer(instance=review,many=True)
